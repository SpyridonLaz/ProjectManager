...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    due_date = serializers.DateTimeField(validators=[EnsureFutureDateValidator, OutOfTimeRangeValidator] )
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate_title(self, value):
        ...
        if Task.objects.filter(title=value,project = project).exists():
            raise serializers.ValidationError(detail={"detail":"Title must be unique!"})
        return value
    ...
    def validate(self, data):
        ...
        OutOfTimeRangeValidator(days=1,value=data['due_date'],)
